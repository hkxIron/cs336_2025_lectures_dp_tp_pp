...
def manual_zero_stage2_gpu(rank: int, world_size: int, data: torch.Tensor, num_layers: int, num_steps: int):
    ...
    * 在 **ZeRO-2** 中，通信发生在 `loss.backward()` **期间**。假设模型有 4 层，当第 4 层的梯度算完时，Hook 立刻触发，把第 4 层的梯度切分并删掉完整的；接着算第 3 层，算完立刻切分并删掉…… 这样，显存中最多只存在**当前正在计算的那一层**的完整梯度。
    ...
    **没有变化。** ZeRO-2 的通信量和 ZeRO-1 完全一样（依然是 $1 \times \text{模型参数量}$）。它只是把原本集中在反向传播之后的通信，**打散**到了反向传播的过程中。这种打散不仅节省了显存，还能让**通信和计算重叠（Overlap）**，在真实的 GPU 环境下反而能提升训练速度！
    ...
    # ==========================================
    for param in params:
        def reduce_scatter_and_free_grad_hook(p):
            if p.grad is None:
                return

            # 1. 分配一块极小的显存，用于接收属于当前 rank 的那 1/world_size 的梯度
            # shape: [local_chunk_size, num_dim]
            local_grad = torch.empty(
                local_chunk_size, num_dim, 
                dtype=p.grad.dtype, 
